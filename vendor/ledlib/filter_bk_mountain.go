...
func NewFilterBkMountain(canvas LedCanvas) LedCanvas {
	...
	filter.filterObjects.Append(NewObjectScrolledBitmap(
		"/asset/image/mountain/mountain1.png", 6, 300*time.Millisecond, true))
	filter.filterObjects.Append(NewObjectScrolledBitmap(
		"/asset/image/mountain/mountain2.png", 7, 300*time.Millisecond, true))
	filter.filterObjectsSnow = NewFilterObjects(canvas)
	filter.filterObjectsSnow.Append(NewObjectScrolledBitmap(
		"/asset/image/mountain/mountain1-s.png", 6, 300*time.Millisecond, true))
	filter.filterObjectsSnow.Append(NewObjectScrolledBitmap(
		"/asset/image/mountain/mountain2-s.png", 7, 300*time.Millisecond, true))
